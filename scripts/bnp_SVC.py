...
# Support Vectro Machine Classifyer
...
#temp_test = StandardScaler().fit_transform(temp_test)
#print(X_train)
#print("## Mean of X_train = ", X_train.mean(axis=0))
#print("## STD of X_train = ", X_train.std(axis=0))

# fit SVC model to the data
print("## Training")
clf = svm.SVC(C=C, probability=True, max_iter=100)
...
p_test = clf.predict_proba(temp_test)

# Producing output of predicted probabilities and writing it into a file
test[test_col_name] = p_test[:,1]
test[[id_col_name,test_col_name]].to_csv("../predictions/pred_SVC_" + timestr + ".csv", index=False)
